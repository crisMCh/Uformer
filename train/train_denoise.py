...
if __name__ == '__main__':  # Ensure multiprocessing safety on Windows
#This allows PyTorch to allocate GPU memory more flexibly, reducing fragmentation.
    ...
    with torch.no_grad():
        ...
        for ii, data_val in enumerate((val_loader), 0):
            ...
            print(f"validation processed {ii} values")
            torch.cuda.empty_cache()
    ...
    for epoch in range(start_epoch, opt.nepoch + 1):
        ...
        # Append the epoch loss to the array
        epoch_losses.append(epoch_loss)
        print(epoch_losses)
    print("Now time is : ",datetime.datetime.now().isoformat())
    # Save the loss array as a .pth file
    loss_file_path = os.path.join(model_dir, "losses")
    torch.save({'epoch_losses': epoch_losses}, loss_file_path)
    print(f"Epoch losses saved to {loss_file_path}")
